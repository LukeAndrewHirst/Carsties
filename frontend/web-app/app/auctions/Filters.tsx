...
const orderButtons = [
    {label: 'Alaphabetical', icon: AiOutlineSortAscending, value: 'make'},
        ...
        <div>
            ...
            <Button.Group>
                {pageSizeButtons.map((value, i) => (
                    <Button key={i} onClick={() => setParams({pageSize: value})} color={`${pageSize === value ? 'red' : 'gray'}`} className='ring-0'>{value}</Button>
                ))}
            </Button.Group>
        </div>
    </div>
